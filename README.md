...
| **Fake solves** | `validateFinalExpr()` on the server parses the expression, verifies each card is used exactly once, and checks the result equals the goal. |
...
| Concern | Status |
|---|---|
| **Client-generated sprint puzzles** | The server does not verify that registered puzzles came from a valid deck, match the sprint band, or are fairly difficult. A scripted client could register trivial puzzles, solve them quickly, and achieve a high score. Mitigation would require server-side puzzle generation. |
| **Session authentication** | `sessionId` (UUID) is the only credential. No login, cookies, or signed tokens. |
...
Creates a new 5-minute sprint session.

**Request:** empty body.

**Response (200):**
```json
{ "sessionId": "uuid", "endsAt": 1710000000000 }
```

`endsAt` is initially `startedAt + 5 minutes`. As puzzles are marked, the server shrinks the remaining budget and updates `endsAt = startedAt + remainingBudget`.

---

### `POST /api/sprint/register`

Registers a puzzle for an active session. Called by the client before the player starts working on each puzzle.
...
Creates or updates a row in `sprint_puzzles` with status `issued`.
...
- **Solved:** validates `finalExpr` against stored cards/goal; increments `sprint_sessions.solved`; deducts `now - issuedAt` from the time budget.
